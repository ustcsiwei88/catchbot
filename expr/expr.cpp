	...
	while(c=getch())
	{
		switch(c)
		{
            ...
            case 'p':
                ...
                {
                    1;//printf("Unable to connect to server. Host not present. Exiting.");
                    return 1;
                }
                break;
            case 's':
                ...
                if (iResult != ErrorCode_OK || pDataDefs == NULL)
                {
                    1;//printf("[SampleClient] Unable to retrieve Data Descriptions.");
                }
                else
                {
                    1;//printf("[SampleClient] Received %d Data Descriptions:\n", pDataDefs->nDataDescriptions);
                }
                }
                break;
            case 'm':	                        // change to multicast
                ...
                iResult = ConnectClient();
                if(iResult == ErrorCode_OK)
                    1;//printf("Client connection type changed to Multicast.\n\n");
                else
                    1;//printf("Error changing client connection type to Multicast.\n\n");
                break;
            case 'u':	                        // change to unicast
                g_connectParams.connectionType = ConnectionType_Unicast;
                iResult = ConnectClient();
                if(iResult == ErrorCode_OK)
                    1;//printf("Client connection type changed to Unicast.\n\n");
                else
                    1;//printf("Error changing client connection type to Unicast.\n\n");
                break;
            case 'c' :                          // connect
                iResult = ConnectClient();
                break;
            case 'd' :                          // disconnect
                ...
                if (iResult == ErrorCode_OK)
                    1;//printf("[SampleClient] Disconnected");
                break;
			default:
				break;
		}
...
int ConnectClient()
{
    ...
    {
        1;//printf("Unable to connect to server.  Error code: %d. Exiting", retCode);
...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data, void* pUserData)
{
    ...
    if ( bSystemLatencyAvailable )
    {
        ...
        1;//printf( "System latency : %.2lf milliseconds\n", systemLatencyMillisec );
        1;//printf( "Total client latency : %.2lf milliseconds (transit time +%.2lf ms)\n", clientLatencyMillisec, transitLatencyMillisec );
    }
    else
    {
        1;//printf( "Transit latency : %.2lf milliseconds\n", transitLatencyMillisec );
    }
 ...
 1;//printf("Rigid Bodies [Count=%d]\n", data->nRigidBodies);
	...
	// Skeletons
 1;//printf("Skeletons [Count=%d]\n", data->nSkeletons);
	for(i=0; i < data->nSkeletons; i++)
	{
		sSkeletonData skData = data->Skeletons[i];
	 1;//printf("Skeleton [ID=%d  Bone count=%d]\n", skData.skeletonID, skData.nRigidBodies);
		for(int j=0; j< skData.nRigidBodies; j++)
		{
			sRigidBodyData rbData = skData.RigidBodyData[j];
		 1;//printf("Bone %d\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\n",
 ...
 1;//printf("Markers [Count=%d]\n", data->nLabeledMarkers);
	for(i=0; i < data->nLabeledMarkers; i++)
	{
	    ...
	    if(marker.z > 0.6)printf(" %3.8f %3.8f %3.8f %3.8f\n",
...
void resetClient()
{
 ...
 1;//printf("\n\nre-setting Client\n\n.");
	...
	if(iSuccess != 0)
	 1;//printf("error un-initting Client\n");
